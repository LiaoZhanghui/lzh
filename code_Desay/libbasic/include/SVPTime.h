 ...
 */
bool getLocalTime(SVPTypeSystemTime& loc_time);

/**
 * \brief Set local time ref to current timezone
 * \param[in, out] loc_time local time
 */
bool setLocalTime(const SVPTypeSystemTime& loc_time);

/**
 * \brief Set system (UTC) time and save time to share memory
 * \param[in] zone_time:system time
 */
bool setZoneTime(const SVPTypeSystemTime& gpsTime);
